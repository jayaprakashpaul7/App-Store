...
class AppStore extends Component {
  state = {activeTabIdItem: tabsList[0].tabId, search: ''}
  ...
  onUpdateTab = tabId => {
    this.setState({activeTabIdItem: tabId})
  }

  getFilterdApps = () => {
    const {activeTabIdItem} = this.state
    const filterd = appsList.filter(each => each.category === activeTabIdItem)
    return filterd
  }

  render() {
    const {activeTabIdItem, search} = this.state
    const filterdApps = this.getFilterdApps()

    const searchfilterdApps = filterdApps.filter(eachAppItem =>
      eachAppItem.appName.toLowerCase().includes(search.toLowerCase()),
    )

    return (
      <div className="bg">
        <h1 className="app-h">App Store</h1>
        ...
        <ul className="tab-list-c">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              tabDetails={eachTab}
              onUpdateTab={this.onUpdateTab}
              isActive={eachTab.tabId === activeTabIdItem}
            />
          ))}
        </ul>

        <ul className="app-list-c">
          {searchfilterdApps.map(eachApp => (
